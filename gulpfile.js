...
var kss = './node_modules/.bin/kss-node ';
 ...
 * Style Guide
 * ==================================== */
gulp.task('styleguide', ['styles'], $.shell.task(
  kss + source + '/scss '+ build + '/styleguide/ --template ' + source + '/styleguide --mask *.scss'
));
...
gulp.task('html-build', function() {
  ...
  return gulp.src([
    ...
    // .pipe($.minifyHtml()) Disabled this because it was ruining my SVGs! :(
    .pipe(gulp.dest(build));
});
...
gulp.task('copyfiles', function() {
    return gulp.src([source + '/**/*.{ttf,woff,eof,svg,ico,eot,css}', source + '/**/*.mp4'])
      .pipe($.plumber(plumberConfig))
      .pipe(gulp.dest(build));
});
...
gulp.task('default', ['clean'], function(){
  runSequence(
    ['html-default', 'images', 'copyfiles', 'styleguide'],
    ['serve']
  );
});
...
gulp.task('build', ['clean'], function(){
  runSequence(
    ['html-build', 'images', 'copyfiles', 'styleguide']
  );
});
...
gulp.task('watch', function() {
  gulp.watch(source + '/scss/**/*.scss', ['styles', 'styleguide', reload]);
  ...
  gulp.watch(source + '/styleguide/**/*', ['styleguide', reload]);
});
